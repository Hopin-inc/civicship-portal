...
- 作成された Firebase テナント ID は `communityPortalConfig` query の返却値（`firebaseTenantId`）から取得できます（civicship-api `config/portal/service.ts`）。
- Firebase Console → Authentication → Identity Platform → 該当テナント
...
> **注意**:
> - LINE 認証情報（Channel Secret / Access Token）はバックエンドでは検証されません。入力値はそのまま保存されるため、フロント側で入力ミスがないか確認してください。
> - フォーム入力の `lineLiffId` から **フロント側 server action（`src/app/sysAdmin/create/features/editor/actions/communityCreate.ts`）が** `liffId`（数字部分）と `liffBaseUrl`（`https://liff.line.me/{liffId}`）を派生させて mutation に渡します。`CommunityLineConfigInput` 自体は `liffId` / `liffBaseUrl` を独立した必須フィールドとして受け取るため、別経路で mutation を直接叩く場合は両方を明示的に渡す必要があります。
> - シークレット系（`accessToken`, `channelSecret`）は SYS_ADMIN 以外のユーザーには resolver で null マスクされます。
