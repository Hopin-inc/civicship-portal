...
- Firebase Console → Authentication → Identity Platform → 自動作成されたテナント
...
> - `lineLiffId` から `liffId`（数字部分）と `liffBaseUrl`（`https://liff.line.me/{liffId}`）が自動生成されます。
...
| `liffAppId` / `liffBaseUrl` | `communityCreate` 時のみ | `CommunityLineConfig` 側のフィールド。更新 mutation 未提供 |
| LINE Channel 情報の更新 | `communityCreate` 時のみ | 同上 |
...
- バックエンドは検証していないため、`communityCreate` 時の入力ミスはエラーにならず保存されます。`liffAppId` 等同様、現状更新 mutation が無いため、修正には civicship-api 側に専用 mutation 追加が必要です。
