...
https://github.com/Hopin-inc/civicship-portal — GPL-3.0.

### Detailed documentation, including UI/UX design principles, integration processes, and user instructions

[Front-end documentation](./frontend-documentation.md) — those three, in that
order, are its three sections.
...
LINE account. See [trying the application](#trying-the-application).

### Fully functional and responsive across different devices

Either link, on a phone, a tablet or a desktop browser. See [trying the
application](#trying-the-application).

## Deliverable 3 — Sharing of a demo video

### The demo video is published on YouTube and is freely accessible to the community

[Twenty-one use-case recordings](./demo/), each listed against the screen it was
recorded on in [where to try each
recording](./demo/README.md#where-to-try-each-recording). YouTube publication is
pending.

### The video includes an access link to the application for usability testing

https://dev.civicship.app/community/neo88 — to be carried in each video's
description on publication.
...
[フロントエンド ドキュメント](./frontend-documentation.md) — この3つが、この順で
そのまま3つの節になっている。
...
いずれかのリンクを、スマートフォン・タブレット・デスクトップブラウザで開く。
[アプリを試す](#アプリを試す)を参照。
...
[ユースケース別21本](./demo/)。各動画がアプリのどの画面に対応するかは
[各録画を試せる画面](./demo/README.md#各録画を試せる画面)に一覧。YouTube への公開は
未了。

### 動画に、使用性テスト用のアプリへのアクセスリンクが含まれること

https://dev.civicship.app/community/neo88 — 公開時に各動画の概要欄に記載する。
