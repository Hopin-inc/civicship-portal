...
export const presenterOpportunityCards = (
  ...
  return edges
    ...
    .map((node) => presenterOpportunityCard(node));
};
...
export const presenterOpportunityCard = (node: GqlOpportunity): ActivityCard => {
  return {
    id: node?.id || "",
    title: node?.title || "",
...
export const presenterOpportunityDetail = (data: GqlOpportunity): ActivityDetail => {
