...
import { AddParticipationPhotosModal } from "@/app/participations/[id]/components/AddParticipationPhotosModal";
import type { Opportunity, Participation } from "@/app/participations/[id]/data/type";

interface ParticipationActionsClientProps {
...
export const ParticipationActionsClient = ({
  opportunity,
  participation,
  ...
  isUploading,
  onAddPhotos,
}: ParticipationActionsClientProps) => {
  return (
    <>
          ...
          <div className="flex items-center justify-between gap-3">
            {participation?.node.status === GqlParticipationStatus.Participated ? (
              <AddParticipationPhotosModal
                trigger={
                  <Button variant="primary" className="w-full" disabled={isUploading}>
