...
export default function ReservationDetailPage({ params }: { params: { id: string } }) {
  ...
  const futureSlotsCount = opportunity?.slots?.filter(slot => 
      ...
      <CardWrapper className="p-4 mb-4">
        ...
        <div>
          ...
          <div className="space-y-2">
            ...
            <p><span className="font-medium">予約した日程:</span> {dayjs(reservation.opportunitySlot?.startsAt).format('YYYY/MM/DD HH:mm')} 〜 {dayjs(reservation.opportunitySlot?.endsAt).format('HH:mm')}</p>
            <p><span className="font-medium">参加人数:</span> {participantCount}名</p>
            <p><span className="font-medium">参加費:</span> {opportunity?.feeRequired || 0} × {participantCount} = {participationFee}円</p>
