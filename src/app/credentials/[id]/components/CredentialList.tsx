...
import { GqlDidIssuanceStatus, GqlGetParticipationQuery, useGetDidIssuanceRequestsQuery, useGetVcIssuanceRequestByEvaluationQuery } from "@/types/graphql";
...
export default function CredentialList(props: OpportunityListProps) {
    const { data } = props;
    const organizerId = data?.participation?.opportunitySlot?.opportunity?.createdByUser?.id;
    const participantId = data?.participation?.user?.id;
    ...
    const organizerDid = didData?.users?.edges
        ?.find(edge => edge?.node?.id === organizerId)
        ?.node?.didIssuanceRequests
        ?.find(req => req?.status === GqlDidIssuanceStatus.Completed)
        ?.didValue;

    const participantDid = didData?.users?.edges
        ?.find(edge => edge?.node?.id === participantId)
        ?.node?.didIssuanceRequests
        ?.find(req => req?.status === GqlDidIssuanceStatus.Completed)
        ?.didValue;
        ...
        <div className="grid grid-cols-1 gap-4 relative">
            <Card className="rounded-2xl border border-gray-200 bg-[#FCFCFC] shadow-none ">
                <CardHeader className="flex flex-row items-center justify-between p-6">
                    ...
                    <div className="text-lg font-bold text-black">{data?.participation?.opportunitySlot?.opportunity?.createdByUser?.name}</div>
            ...
            <CardHeader className="flex flex-row items-center justify-between p-4 px-6">
                ...
                <div className="flex items-center flex-1 min-w-0 ml-8">
                    <span className="font-bold text-black whitespace-nowrap overflow-hidden text-ellipsis text-sm flex-1">
                        {data?.participation?.opportunitySlot?.opportunity?.description}
                    </span>
                    <Link
            ...
            <Card className="rounded-2xl border border-gray-200 bg-[#FCFCFC] shadow-none ">
                <CardHeader className="flex flex-row items-center justify-between p-6">
                    ...
                    <div className="flex flex-col items-end">
                    <div className="text-lg font-bold text-black">{data?.participation?.user?.name}</div>
                        <div className="flex items-center text-gray-400 text-sm mt-1">
                            <Copy 
                                className="w-4 h-4 mr-1" 
                                onClick={() => {
            ...
            <div className="text-gray-400 text-sm text-right mt-4">
                発行日 {vcData?.vcIssuanceRequests.edges[0]?.node?.completedAt
                    ? formatDateTime(vcData?.vcIssuanceRequests.edges[0]?.node?.completedAt, "yyyy年M月d日")
                    : "発行中..."}
            </div>
        </div>
    )
