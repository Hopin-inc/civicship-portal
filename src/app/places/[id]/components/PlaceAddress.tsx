...
const PlaceAddress = ({ detail }: { detail: IPlaceDetail }) => {
  // #NOTE: strapi に保存された緯度経度は若干位置がずれるため、住所からマップを表示している
  const mapUrl = `https://www.google.com/maps/embed/v1/place?key=${process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY}&q=${encodeURIComponent(detail.address)}`;
  // const mapUrl =
