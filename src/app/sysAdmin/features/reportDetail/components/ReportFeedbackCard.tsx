...
/**
 * Report detail 内で 1 件の feedback を表示する card。
 * 文脈上「どの Report か」は自明なので、テンプレ詳細用 `FeedbackCard` と
 * 違って Report リンクは含まない。
 */
export function ReportFeedbackCard({ feedback }: Props) {
  return (
    <article className="space-y-2 border-b border-muted py-3 last:border-b-0">
      ...
      {(feedback.feedbackType || feedback.sectionKey) && (
