...
export const useFirebaseAuthState = ({
  ...
  useEffect(() => {
    ...
    const unsubscribe = lineAuth.onAuthStateChanged(async (user) => {
      ...
      // 同一ユーザー・同一テナントの場合のみ早期 return（Copilot指摘: null===null でスキップしないよう両者が非 null の場合に限定）
      ...
      if (user) {
        ...
        if (expectedTenantId && user.tenantId !== expectedTenantId) {
          ...
          // Devin指摘: signOut 後に return するだけでは setState が呼ばれず、
          // onAuthStateChanged(null) が来ても早期 return に引っかかって loading のまま固まる。
          // ここで確実に未認証状態へ遷移させる。
