...
export class AuthRedirectService {
  ...
  public isPathInSignUpFlow(pathname: string): boolean {
    ...
    // 変更後:
    const phoneVerificationRequiredPaths = ["/sign-up"]; // ← ここだけにする
  ...
  public getRedirectPath(
    ...
    if (this.isProtectedPath(pathname)) {
      switch (authState) {
        ...
        // 変更前: line_* は /sign-up/phone-verification へ
        // 変更後: まとめて /sign-up へ寄せる
    ...
    if (this.isPathInSignUpFlow(pathname)) {
      switch (authState) {
        ...
        // 変更前: line_* は /sign-up/phone-verification へ
        // 変更後: /sign-up に寄せる（既に /sign-up なら滞在）
        case "line_authenticated":
        case "line_token_expired":
        case "phone_authenticated":
        case "phone_token_expired":
  ...
  public getPostLineAuthRedirectPath(nextPath: RawURIComponent | null): RawURIComponent {
    ...
    switch (authState) {
      ...
      // 変更前: line_authenticated / phone_token_expired → /sign-up/phone-verification
      // 変更後: まとめて /sign-up
      case "line_authenticated":
      case "phone_token_expired":
      case "phone_authenticated":
      ...
      case "user_registered":
      default:
        return (nextPath ?? "/") as RawURIComponent; // ← TS2366 回避のため常に return
    }
  }
